...
| ----------- | ------ | ------------- |-------------------------------| ----------------------------------- | ----------------------------------------------- |
| 회원가입        | POST   | `/api/users`  | 사용자 계정 생성                     | `email`, `password`, `username`     | `201 Created`<br>`400`: 유효성 오류<br>`409`: 중복 이메일 |
| 로그인 (토큰 발급) | POST   | `/api/tokens` | 로그인 및 액세스/리프레시 토큰 발급          | `email`, `password`                 | `200 OK`: 토큰 및 사용자 정보 반환<br>`401`: 인증 실패        |
| 로그아웃        | DELETE | `/api/tokens` | 로그아웃 (리프레시 토큰 폐기)             | 헤더 또는 쿠키의 `refreshToken` 또는 body 전달 | `204 No Content`: 성공<br>`401`: 인증 실패 또는 토큰 없음   |
| 토큰 재발급      | PUT    | `/api/tokens` | 리프레시 토큰으로 액세스 토큰, 리프레시 토큰 재발급 | `refreshToken`                      | `200 OK`: 새 액세스 토큰 발급<br>`401`: 유효하지 않은 토큰      |
