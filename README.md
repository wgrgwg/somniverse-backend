...
| ---- | ---- | ------------------ | ------------ | ------------------------------- | --------------------------------------------------- |
| 회원가입 | POST | `/api/auth/signup` | 사용자 계정 생성    | `email`, `password`, `username` | `201 Created`: 성공<br>`400`: 유효성 오류<br>`409`: 중복 이메일 |
| 로그인  | POST | `/api/auth/login`  | 로그인 및 JWT 발급 | `email`, `password`             | `200 OK`: 토큰 및 사용자 정보 반환<br>`401`: 인증 실패            |
